...
import { useAppBridgeReady } from '../hooks/useAppBridgeReady';
import { usePolarisReady } from '../hooks/usePolarisReady';
import { useShopifyAuth } from '../hooks/useShopifyAuth';

/**
 * Top-level Shopify embedded app UI.
 * Waits for {@link useShopifyAuth}, {@link useAppBridgeReady}, and {@link usePolarisReady} before rendering Polaris demo content.
 * Shows spinners while auth or scripts load, an error banner when auth fails, and the sample merchant homepage when ready.
 * Non-embedded visitors see a warning banner because admin chrome requires opening from Shopify Admin.
 * @returns {import('react').ReactElement} Full-page React tree for the current auth and loading state.
 */
export function App() {
  const auth = useShopifyAuth();
  const appBridgeReady = useAppBridgeReady();
  const polarisReady = usePolarisReady();
  ...
  if (!appBridgeReady || !polarisReady) {
    return (
      <div className="auth-loading">
        <s-spinner size="large" />
      </div>
  ...
  if (auth.status === 'error') {
    return (
      <>
        <AppNav />
        <s-page heading="Shopify Boilerplate" inline-size="base">
          <s-section>
            <s-banner tone="critical" heading="Authentication error">
              {auth.message}
            </s-banner>
          </s-section>
        </s-page>
    ...
    <>
      ...
      {!auth.embedded ? (
        <s-banner tone="warning">
          Open this app from Shopify Admin to render the page title and actions in the admin header.
        </s-banner>
      ) : null}
      <s-page slot="heading" heading="hello world" inline-size="base">
        ...
        {/* === */}
        {visible.banner && (
          <s-banner dismissible onDismiss={() => setVisible({ ...visible, banner: false })}>
            3 of 5 puzzles created. <s-link href="#">Upgrade to Puzzlify Pro</s-link> to create
            unlimited puzzles.
          </s-banner>
        )}
                ...
                <s-box>
                  <s-grid gridTemplateColumns="1fr auto" gap="base" padding="small">
                    <s-checkbox
                      ...
                      onInput={(inputEvent) =>
                        setProgress(
                          inputEvent.currentTarget.checked ? progress + 1 : progress - 1,
                        )
                      }
                    ></s-checkbox>
                    <s-button
        ...
        <s-section>
          ...
          <s-grid gridTemplateColumns="repeat(auto-fit, minmax(240px, 1fr))" gap="base">
            ...
            >
              ...
              <s-paragraph>
                Now you can fine-tune the difficulty of your puzzles with new rotation controls,
                edge highlighting options, and piece recognition settings.
              </s-paragraph>
            </s-grid>
          </s-grid>
          <s-stack
